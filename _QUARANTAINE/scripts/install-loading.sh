#!/usr/bin/env bash
# =============================================================================
# install-loader.sh — Pullman Dakar Teranga
# Installe le loading page (overlay) sur TOUTES les pages HTML
# -----------------------------------------------------------------------------
# Ce script :
#   1. Supprime les anciennes injections loading-overlay (nettoyage)
#   2. Crée css/loading-overlay.css
#   3. Crée js/loading-overlay.js
#   4. Injecte proprement dans TOUS les .html (sauf gabarit/loading)
#   5. Vérifie que l'injection est correcte dans chaque page
#   6. Affiche un rapport détaillé
#
# Usage :
#   bash install-loader.sh             # installation
#   bash install-loader.sh --verify    # vérifie sans modifier
#   bash install-loader.sh --clean     # supprime tout
# =============================================================================
...
# --- Couleurs ---
if [ -t 1 ]; then
    R=$'\033[0m'; B=$'\033[1m'; RD=$'\033[31m'; GR=$'\033[32m'
    YL=$'\033[33m'; BL=$'\033[34m'; CY=$'\033[36m'; DM=$'\033[2m'
else
    R=""; B=""; RD=""; GR=""; YL=""; BL=""; CY=""; DM=""
fi
...
if [ "$MODE" = "clean" ]; then
    ...
    ok "Fichiers css/js supprimés"

    for f in *.html; do
        [ -f "$f" ] || continue
        [ "$f" = "$SKIP" ] && continue
        # Retirer les lignes d'injection
        grep -v "loading-overlay" "$f" > "$f.tmp" && mv "$f.tmp" "$f"
        printf '  %s•%s %s nettoyé\n' "$YL" "$R" "$f"
    done
    ok "Terminé."
    exit 0
fi
...
if [ "$MODE" = "verify" ]; then
    head "Vérification"
    [ -f "$CSS_FILE" ] && ok "CSS : $(wc -c < "$CSS_FILE" | tr -d ' ') octets" || err "CSS manquant"
    [ -f "$JS_FILE" ]  && ok "JS  : $(wc -c < "$JS_FILE" | tr -d ' ') octets"  || err "JS manquant"
    echo ""
    for f in *.html; do
        [ -f "$f" ] || continue
        [ "$f" = "$SKIP" ] && continue
        c=$(grep -c "$CSS_MARK" "$f" 2>/dev/null | head -1)
        j=$(grep -c "$JS_MARK"  "$f" 2>/dev/null | head -1)
        if [ "$c" -ge 1 ] && [ "$j" -ge 1 ]; then
            printf '  %s✓%s %s (css=%s js=%s)\n' "$GR" "$R" "$f" "$c" "$j"
        else
            printf '  %s✗%s %s (css=%s js=%s)\n' "$RD" "$R" "$f" "$c" "$j"
        fi
    done
...
echo ""
printf '%s╔══════════════════════════════════════════════════════════╗%s\n' "${B}${CY}" "$R"
printf '%s║   Pullman — Installation du loading page                 ║%s\n' "${B}${CY}" "$R"
printf '%s╚══════════════════════════════════════════════════════════╝%s\n' "${B}${CY}" "$R"

# --- Vérif index.html présent ---
[ -f "index.html" ] || { err "index.html introuvable — lance depuis la racine"; exit 1; }
...
# 1. CSS
# =============================================================================
head "1/4 — Fichier CSS"

cat > "$CSS_FILE" << 'CSS_EOF'
/* ============================================================= */
/* LOADING-OVERLAY.CSS — Pullman Dakar Teranga                   */
...
#rp-loader.rp-is-active {
    opacity: 1;
    visibility: visible;
    pointer-events: auto;
}
#rp-loader .rp-loader-inner {
    display: flex;
...
#rp-loader.rp-is-active .rp-loader-inner {
...
.rp-loader-bar-fill {
    ...
    width: 0%;
    background: #47B27D;
    box-shadow: 0 0 12px rgba(71,178,125,.75);
...
ok "CSS créé ($(wc -c < "$CSS_FILE" | tr -d ' ') octets)"
...
cat > "$JS_FILE" << 'JS_EOF'
/* ============================================================= */
/* LOADING-OVERLAY.JS — Pullman Dakar Teranga                    */
/* Affiche un écran de chargement entre les pages                */
/* ============================================================= */
(function () {
    'use strict';
    ...
    var LOADER_HTML =
      '<div class="rp-loader-inner">' +
        ...
        '<svg class="rp-wordmark" viewBox="670.34 536.84 331.73 53.31" width="212" height="34" xmlns="http://www.w3.org/2000/svg">' +
          '<path d="M674.77,569.72V590.1H670.5c0-.49-.1-1-.1-1.55,0-8.92-.07-17.84-.06-26.76,0-6.24,3.23-10.8,9.16-12.74a26.81,26.81,0,0,1,17.16,0c7,2.34,10.51,8.53,8.61,15.1-1.46,5-5.35,7.58-10.1,8.91C688.13,575.06,682.38,574.08,674.77,569.72Z"/>' +
        ...
        '</div>' +
        '<div class="rp-loader-word"><span class="bold">Yeksil Ak Jaam</span> <span class="reg">« You are welcome »</span></div>' +
      '</div>';

    var loader = null, barFill = null, pctLabel = null, startTime = 0;
    ...
    function startProgress() {
        ...
        (function tick(now) {
            ...
            var p = Math.floor((1 - Math.pow(1 - t, 1.8)) * 100);
    ...
    function isInternalLink(anchor) {
        ...
        try {
            ...
            if (/\.(pdf|jpg|jpeg|png|gif|webp|svg|zip|mp4|mp3)$/i.test(url.pathname)) return false;
...
ok "JS créé ($(wc -c < "$JS_FILE" | tr -d ' ') octets)"

# =============================================================================
# 3. INJECTION
# =============================================================================
head "3/4 — Injection dans les HTML"
...
for f in *.html; do
    [ -f "$f" ] || continue
    [ "$f" = "$SKIP" ] && continue
    [ "$f" = "loading.html" ] && continue

    # Nettoyer les anciennes injections
    grep -v "loading-overlay" "$f" > "$f.tmp" 2>/dev/null && mv "$f.tmp" "$f"

    # Backup
    cp -p "$f" "$f.bak" 2>/dev/null

    # Injecter CSS avant </head>
    awk -v line="$CSS_LINE" '
        BEGIN { done=0 }
        /<\/head>/ && !done { print line; done=1 }
        { print }
    ' "$f" > "$f.tmp" && mv "$f.tmp" "$f"
    ...
    awk -v line="$JS_LINE" '
        BEGIN { done=0 }
        /<\/body>/ && !done { print line; done=1 }
        { print }
    ' "$f" > "$f.tmp" && mv "$f.tmp" "$f"

    # Vérifier
    c=$(grep -c "$CSS_MARK" "$f" | head -1)
    j=$(grep -c "$JS_MARK" "$f" | head -1)

    if [ "$c" -ge 1 ] && [ "$j" -ge 1 ]; then
        printf '  %s✓%s %-35s css=%s js=%s\n' "$GR" "$R" "$f" "$c" "$j"
        added=$((added+1))
    else
        printf '  %s✗%s %-35s css=%s js=%s\n' "$RD" "$R" "$f" "$c" "$j"
    fi
...
ok "$added page(s) patchée(s)"

# =============================================================================
# 4. VÉRIFICATION FINALE
# =============================================================================
head "4/4 — Vérification finale"
...
ok "$PAGES_OK page(s) avec le loader complet"
...
echo "════════════════════════════════════════════════════════════"
echo ""
echo "  ${B}Pour tester :${R}"
echo "    1. Ouvre index.html dans Chrome"
echo "    2. Fais Ctrl + Shift + R (hard refresh)"
echo "    3. Clique sur 'CHAMBRES' dans le menu"
echo "    → Le loader apparaît 3.4 secondes"
echo ""
echo "  ${B}Pour ajuster la durée :${R}"
echo "    Édite js/loading-overlay.js ligne 8 :"
echo "    var DURATION = 3400;   ← en millisecondes"
echo ""
echo "  ${B}Pour vérifier :${R}"
echo "    bash install-loader.sh --verify"
echo ""
echo "  ${B}Pour tout supprimer :${R}"
echo "    bash install-loader.sh --clean"
echo ""
