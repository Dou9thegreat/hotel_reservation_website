/* ============================================================= */
/* EXTRAS.JS v7 — Vraies photos, aucune icone                    */
/* ============================================================= */
(function () {
    'use strict';

    /* ---------- CATALOGUE ENRICHI ---------- */
    /* Chaque extra pointe vers une VRAIE image de votre projet */
    var EXTRAS = [
        /* SPA */
        { id: 'spa-massage', cat: 'spa',
    ...
    /* ---------- BANNIÈRES (utilisant des photos existantes) ---------- */
    var BANNERS = {
    ...
    /* ---------- BANDEAU : 4 EXTRAS POPULAIRES (VIGNETTES PHOTO) ---------- */
    function renderQuickGrid() {
        ...
        grid.innerHTML = popular.map(function (e) {
            ...
            return '<button type="button" class="extras-quick' + active + '" data-quick-id="' + e.id + '">' +
                '<span class="extras-quick-thumb" style="background-image:url(\'' + e.image + '\');"></span>' +
    ...
    function renderList() {
        ...
        list.innerHTML = items.map(function (e) {
            ...
            return '<div class="extras-card' + sel + '" data-extra-id="' + e.id + '">' +
                '<div class="extras-card-image" style="background-image:url(\'' + e.image + '\');"></div>' +
                '<div class="extras-card-body">' +
                    '<span class="extras-card-name">' + e.name + '</span>' +
                    '<span class="extras-card-desc">' + e.desc + '</span>' +
                    '<span class="extras-card-price">+' + fmt(e.price) + '</span>' +
                '</div>' +
                '<span class="extras-card-check"><i class="fa-solid fa-check"></i></span>' +
            '</div>';
        }).join('');

        list.querySelectorAll('.extras-card').forEach(function (el) {
